	...
	data := []struct {
	...
	}{
		{
			...
			answers: []string{"ваше питання таке цікаве, навіть не знаю, як на нього відповісти",
				...
				"я сьогодні грайливий, з такими питаннями до chat gpt", "пикол зайшов занадто далеко"},
		...
		{
			topic:         "ні",
			templates:     []string{"ні", "ні , не погоджуюсь", "не погоджуюсь"},
			answers:       []string{"ну ні так ні , питайте щось ще"},
			singleInserts: []string{""},
			groupInserts:  []string{""},
		},
		{
		...
		},
		{
			...
			answers:       []string{"є пропозиція сходити $", "як варіант сходити $", "зараз в тренді сходити $", "пропоную вам $"},
			singleInserts: []string{""},
